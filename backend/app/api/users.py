...
def get_user(id):
    # TODO: allow admins to get email address
    include_email = g.current_user.id == id
    return jsonify(
        User.query.get_or_404(id).to_dict(
...
def create_user():
    ...
    db.session.add(user)
    db.session.commit()
    response = jsonify(user.to_dict(include_email=True, include_token=True))
